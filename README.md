...
- All offsets centralized in a single `Config` struct (`Config.cpp`)
...
4. Load Config (default offsets for detected architecture)
...
- Update the defaults in `Config.cpp` for your version and recompile
...
- Update the values in `Config.cpp` and recompile
