...
Added framework for loading configuration from JSON files:
- Optional `roblox_config.json` file support
- Sensible defaults when file not found
- Extensible for future JSON parsing improvements

**Location:** `LoadOrCreateConfig()` function, lines ~165-180
...
### 2. `roblox_config_example.json`
Template configuration file showing:
- Map/string/node offset structure
- 32-bit vs 64-bit configurations
- Scanning parameters
- Version-specific extension points
- Usage instructions
...
3. Test with custom `roblox_config.json` (manual offset changes)
